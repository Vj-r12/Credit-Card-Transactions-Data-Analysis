...
-- Fetch the top 5 city by using total card type.

SELECT DISTINCT CITY,
	COUNT(CARD_TYPE) AS SUM_OF_CARD_TYPE
FROM CREDIT_CARD_TRANSACTION
GROUP BY CITY 
ORDER BY SUM_OF_CARD_TYPE DESC
LIMIT 5;
...
-- Count the expenses type. Which card type as used to pay highest number of transactions for expenses type.
...
WITH CTE AS
	(SELECT CARD_TYPE,
			AMOUNT,DATE,
	CASE
		WHEN EXTRACT(MONTH FROM date) BETWEEN 1 AND 3 THEN 'Qtr1'
		WHEN EXTRACT(MONTH FROM date) BETWEEN 4 AND 6 THEN 'Qtr2'
		WHEN EXTRACT(MONTH FROM date) BETWEEN 7 AND 9 THEN 'Qtr3'
	        WHEN EXTRACT(MONTH FROM date) BETWEEN 10 AND 12 THEN 'Qtr4'
